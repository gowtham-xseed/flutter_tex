...
var isFormulaRenderedRecently = false;
        ...
        if (e.target.innerHTML && e.target.innerHTML.search(/flt-platform-view/)) {
            ...
            if(platformViews && platformViews.length) {
                platformViews.forEach((platformView) => {
                    ...
                    if(view && listeningPlatformViews.includes(view.id)) {
                        // if(!isFormulaRenderedRecently) {
                            isFormulaRenderedRecently = true;
                            maxRetry = 30;
                            initWebTeXView(view.id.replace('tex_view_', ''), rawData, true);

                            setTimeout(() => {
                                isFormulaRenderedRecently = false;
                            }, 1000);
                        // }
                    }
                });
...
function initWebTeXView(viewId, rawData, forceReRender= false) {
    ...
    var lastData;
    var retryTimer;
    if(listeningPlatformViews.length == 0 || !listeningPlatformViews.includes('tex_view_' + viewId)) {
        listeningPlatformViews.push('tex_view_' + viewId);
    }
